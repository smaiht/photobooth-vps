...
Команда `/get_config` возвращает администратору один читаемый документ
`photobooth_configs.txt`: сначала секцию с актуальным `config_app.json`, затем
секцию с `config_camera.json`. Он доставляется тем же способом, что и `/logs`,
после чего временная копия удаляется с Диска. Команда ничего не меняет и не
перезапускает будку.
